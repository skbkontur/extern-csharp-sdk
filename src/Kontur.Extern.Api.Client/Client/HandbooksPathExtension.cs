...
public static class HandbooksPathExtension
{
    ...
    public static async Task<FnsFormsPage> GetFnsForms(this HandbooksPath path, int skip, int take, TimeSpan? timeout = null)
    {
        var apiClient = path.Services.Api;
        var fnsForms = await apiClient.Handbooks.GetFnsForms(skip, take, timeout).ConfigureAwait(false);
        return fnsForms;
    }
}