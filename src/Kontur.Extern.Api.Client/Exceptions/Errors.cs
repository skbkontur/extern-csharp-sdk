...
namespace Kontur.Extern.Api.Client.Exceptions
{
    internal static class Errors
    {
        ...
        public static ArgumentException InvalidOgrn([InvokerParameterName] string paramName, string value) =>
            new($"The given OGRN '{value}' does not match to one of OGRN formats. The OGRN should have 15 digits code (for individual entrepreneur) or 13 digits code (for legal entity)", paramName);

        public static ArgumentException InvalidSnils([InvokerParameterName] string paramName, string value) =>
            new($"The given SNILS '{value}' does not match to one of SNILS formats. The SNILS should have 11 digits code or should have format 'ХХХ-ХХХ-ХХХ XX'", paramName);
        ...
        private static string GetFormatName(AuthorityNumberKind numberKind)
        {
            return numberKind switch
            {
                ...
                AuthorityNumberKind.Ogrn => "OGRN",
                AuthorityNumberKind.Snils => "SNILS",
                ...
                AuthorityNumberKind.SfrRegNumber => "SFR reg number",
        ...
        public static Exception UnsuccessfulApiResponse(ApiError apiErrorResponse) =>
            new ApiException(apiErrorResponse);
