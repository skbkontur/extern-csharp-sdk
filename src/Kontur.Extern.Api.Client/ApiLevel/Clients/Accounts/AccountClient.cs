...
namespace Kontur.Extern.Api.Client.ApiLevel.Clients.Accounts
{
    public class AccountClient : IAccountClient
    {
        ...
        public Task<AccountList> GetAccountsAsync(int? skip = null, int? take = null, TimeSpan? timeout = null, bool showBlocked = false)
        {
            var url = new RequestUrlBuilder("v1")
                .AppendToQuery("skip", skip)
                .AppendToQuery("take", take)
                .AppendToQuery("showBlocked", showBlocked)
                .Build();
            return HttpRequestFactory.GetAsync<AccountList>(url, timeout);
        }

        public Task<Account> GetAccountAsync(Guid accountId, TimeSpan? timeout = null, bool showBlocked = false) =>
            HttpRequestFactory.GetAsync<Account>(new RequestUrlBuilder("v1").AppendToPath(accountId).AppendToQuery("showBlocked", showBlocked).Build(), timeout);

        public Task<Account?> TryGetAccountAsync(Guid accountId, TimeSpan? timeout = null, bool showBlocked = false) =>
            HttpRequestFactory.TryGetAsync<Account>(new RequestUrlBuilder("v1").AppendToPath(accountId).AppendToQuery("showBlocked", showBlocked).Build(), timeout);
